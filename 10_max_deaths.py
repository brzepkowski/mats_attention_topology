...
def persistence_entropy(persistence_intervals_multiple_prompts, homology_dim, ax):
    ...
    for layer_idx in tqdm(range(model.config.num_hidden_layers)):
        ...
        pe_array = max_death(_persistence_intervals)
...
if __name__ == "__main__":
    ...
    for i, fig in enumerate(figs):
        ...
        if PREFIX:
            fig.suptitle(rf"{subtitle} | $\max(death)_{{{i}}}$ | prefix: True")
            fig.savefig(f"{subtitle}_MaxDeath{i}_n{TEST_SIZE}_pT.png")
            fig.savefig(f"{subtitle}_MaxDeath{i}_n{TEST_SIZE}_pT.pdf")
        else:
            fig.suptitle(rf"{subtitle} | $\max(death)_{{{i}}}$ | prefix: False")
            fig.savefig(f"{subtitle}_MaxDeath{i}_n{TEST_SIZE}_pF.png")
            fig.savefig(f"{subtitle}_MaxDeath{i}_n{TEST_SIZE}_pF.pdf")
