...
CORRECT_PROMPTS_PATH = "prompts/Qwen3-4B/correct_prompts.json"
CONFLICTING_PROMPTS_PATH = "prompts/Qwen3-4B/conflicting_prompts.json"
...
def max_lifespan(arr):
    ...
    max_lifespans = []
    for single_prompt_persistence_intervals in arr:
        lifespans = np.array([bar[1] - bar[0] for bar in single_prompt_persistence_intervals])
        max_lifespan = np.max(lifespans)
        max_lifespans.append(max_lifespan)
    return np.array(max_lifespans)
...
def persistence_entropy(persistence_intervals_multiple_prompts, homology_dim, ax):
    ...
    for layer_idx in tqdm(range(model.config.num_hidden_layers)):
        ...
        pe_array = max_lifespan(_persistence_intervals)
...
if __name__ == "__main__":
    ...
    for i, fig in enumerate(figs):
        ...
        if PREFIX:
            fig.suptitle(rf"{subtitle} | $\max(lifespan)_{{{i}}}$ | prefix: True")
            fig.savefig(f"{subtitle}_MaxLifespan{i}_n{TEST_SIZE}_pT.png")
            fig.savefig(f"{subtitle}_MaxLifespan{i}_n{TEST_SIZE}_pT.pdf")
        else:
            fig.suptitle(rf"{subtitle} | $\max(lifespan)_{{{i}}}$ | prefix: False")
            fig.savefig(f"{subtitle}_MaxLifespan{i}_n{TEST_SIZE}_pF.png")
            fig.savefig(f"{subtitle}_MaxLifespan{i}_n{TEST_SIZE}_pF.pdf")
